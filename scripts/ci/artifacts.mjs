...
export function assetNames(version) {
  ...
  return [`NotchUsage_${version}_universal.dmg`, `NotchUsage_${version}_universal.app.zip`, "build-info.json", "SHA256SUMS.txt"];
}
...
export function verifyArtifacts(directory, { version, commit, minimumMacOS = "13.0" }) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify([...expected].sort())) throw new Error("Arquivos ausentes ou inesperados no pacote de release.");
  const assets = expected.map((name) => {
    const path = join(directory, name);
    const stat = lstatSync(path);
    if (!stat.isFile() || !stat.size) throw new Error(`Arquivo inválido: ${name}`);
  ...
  if (info.schemaVersion !== 1 || info.version !== version || info.commit !== commit || info.tag !== `v${version}` ||
    ...
    throw new Error("O pacote não corresponde à versão, commit, plataforma ou assinatura esperados.");
  }
  for (const asset of assets.slice(0, 2)) {
    if (info.files?.[asset.name]?.sha256 !== asset.digest.slice(7) || info.files[asset.name].bytes !== asset.size) {
      throw new Error(`Metadados de integridade inválidos: ${asset.name}`);
    }
  }
  const sums = assets.slice(0, 3).map((a) => `${a.digest.slice(7)}  ${a.name}\n`).join("");
  if (assets[3].data.toString("utf8") !== sums) throw new Error("SHA256SUMS.txt não corresponde aos arquivos.");
  return { info, assets };
}
